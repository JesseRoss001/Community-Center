...
Images will be directly uploaded to the gallery with the option to delete 
![Screenshot 2023-12-06 025130](https://github.com/JesseRoss001/Community-Center/assets/79084912/6337b208-0af4-4748-95c0-df73645cd2a2)
...
Created events will show on instructor home page . 
...
They can be updated only if the capacity is not reduced by members joining .
...
Images can also be reuploaded to gallery in updates.
...
Events can be deleted and a refund issued if no users have joined your events yet . 
Events cannot be deleted upon memeber joining . 
Graph displays balance statements .
...
Users register without info for card number bagde or staff id . They do not have permission to create events . 
...
Users can join events from the booking page . 
...
This then shows on their home page along with a history of transactions and a graph display . 
...
Users cannot rejoin the same event twice .   
...
Users can like any event even if not joined to it and unlike at will . Only users who have joined an event can rate the instructor 
...
Event likes are dynamically updated across site as well as instructor ratings . 
...
Users budget cannot exceed -28 before in person credits from staff members issued. 
Bug fixed . 
...
Search Events known error upon clicking find event it does not take you to the specific event but just the page where the event is located. not enough time to fix this error . 
Works with combinations too 
Search by tag 
...
Descending and ascending like count . 
...
Search by time of day day of week or combination . 
...
Find event with known error . Takes to the correct page but not the correct event . 
