...
class EventImageFilter(admin.SimpleListFilter):
    """
    A custom filter for the Django admin interface to filter events based on whether they have an image.
    """
    title = _('Has Image')
    ...
    def lookups(self, request, model_admin):
        """
        Returns a list of tuples. The first element in each tuple is the coded value for the option 
        that will appear in the URL query. The second element is the human-readable name for the option 
...
class EventAdmin(SummernoteModelAdmin):
    ...
    def get_urls(self):
        ...
        urls = super().get_urls()
        custom_urls = [
            path('delete-image/<int:event_id>/', self.admin_site.admin_view(self.delete_image), name='delete-event-image'),
        ]
    ...
    def delete_image_link(self, obj):
        ...
        """
        if obj.image:
            return format_html('<a href="{}">Delete Image</a>', reverse('admin:delete-event-image', args=[obj.pk]))
        return "No Image"
    ...
    def delete_image(self, request, event_id):
        ...
        This view is called when the delete link is clicked in the admin interface.
        """
...
class BalanceChangeAdmin(admin.ModelAdmin):
    ...
    Provides an interface to view details of balance changes including the user profile,
